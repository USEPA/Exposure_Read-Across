...
counter=0
def generate_ecfp(smiles: str):# -> numpy.ndarray
    """"
...
# Am not using the terminology of "New Chemical" and "Existing Chemical" 
# because my app is currently divided between providing the structural analogs of a substance
# or providing data on a substance. It is possible for the new chemical to be in DSSTox. 
# Thus, saying that only existing chemicals (in the sense of not being in TSCA) 
# can be found in the tab displaying substances in DSSTox isn't correct 
...
with tab1:
    ...
    if smiles_ready:    
        ...
        NNi["sim"] = 1-NNi['sim']
        NNi.drop(columns=['index'], inplace=True )
        NNi.rename(columns={'ID':'DTXSID', 'sim':'Similarity'}, inplace=True)
