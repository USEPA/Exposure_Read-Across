...
class AnalogClass:
    ...
    def ppm_to_mg(self, value, unit, mw) -> float:
        ...
        Args:
            value (float or int): The numerical value of the air concentration
    ...
    def AnalogFinder(self, MFPs_address, chem_info_address,
        ...
        Args:
            MFPs_address (str): File address of the set of morgan fingerprints 
            from which analogs are drawn

            chem_info_address (str): File address of the set of additional
            information on the chemicals from which analogs are drawn
        ...
        # Chose feather as the datatype because it was the fastest datatype
        # to read and write
        if str(MFPs_address).endswith(".feather"):
            FP1 = pd.read_feather(MFPs_address)
        # The files for unit testing are .parq type,
        # requiring the conditional below
    ...
    def analog_retrieve(self, script_location, smiles_code):
        ...
        with st.container(border=True):
            ...
            st.markdown('Analogs are calculated using DSSTox; '
                        'QSAR-Ready SMILES are '
    ...
    def osha_summary_fig(self, returned_table, osha_data):
        ...
        analog_htmp = alt.Chart(input_set).mark_rect().encode(
            # Line not aligned with open parentheses due to this
    ...
    def cdr_input(self, cdr_set, main_set,
                  dsstoxid, substance_name, data_title):
    ...
    def osha_cpdat_cdr(self, returned_table, osha_data, run_from_location):
        ...
        cdr_ccu = pd.read_parquet(run_from_location/'data'/
                                  'cdr_ccu_plus_dtxsids.parq')
        cdr_ipu = pd.read_parquet(run_from_location/'data'/
                                  'cdr_ipu_plus_dtxsids.parq')
        cdr_mi = pd.read_parquet(run_from_location/'data'/
                                 'cdr_mi_plus_dtxsids.parq')

        AnalogClass.analog_container.header("Summary of Structural Analog Data")
        AnalogClass.analog_container.markdown("Availability of data on analogs"
                                              " from CPDat, CDR, and OSHA.")
        AnalogClass.analog_container.markdown("Chemical Data Reporting (CDR)"
                                              " consists of data reported from"
                                              " manufacturers and importers "
                                              "on the production volume and "
                                              "uses of chemicals on the US "
                                              "market.")
    ...
    def exp_pthwy(self, returned_table, run_from_location):
        ...
        ip_htmp = alt.Chart(ip_input).mark_rect().encode(
            x=alt.X('substance_name:N', 
                    axis=alt.Axis(title=''),
                    scale=alt.Scale(domain=list(ip_input['substance_name']
                                                .unique()))),
            y=alt.Y('Exposure_Type:N', title=''),
            color=alt.Color('Exposure_Probabilities:Q')
                     .scale(scheme='plasma')
        ...
        (AnalogClass
         .analog_container
         .markdown('##### Estimated probability of exposure '
                   'through common pathways'))
        (AnalogClass
         .analog_container
         .markdown('##### Estimates of probability '
                   'below 0.5 have been made blank'))
    ...
    def model_prediction(self, returned_table, run_from_location):
        ...
        for dtxsid, chem_name in zip(returned_table['DTXSID'],
            ...
            # because each input must be filtered independently 
        ...
        third_fig_input['log_exposure_dose'] = third_fig_input['exposure_dose'].apply(
            lambda x: self.null_replacer(x, min_val))
                                                
        x_scale_domain = third_fig_input['substance_name'].unique().tolist()
        modeled_htmp = (
            alt.Chart(third_fig_input)
                .mark_rect()
                .encode(x=alt.X(
                                'substance_name:N',
                                axis=alt.Axis(title=''),
                                scale=alt.Scale(domain=x_scale_domain)
                                ),
                        y=alt.Y('database:N', title=''),
                        color=alt.Color('log_exposure_dose:Q'
                                        ).scale(type="log")
                                         .title(['(mg/kg/day)'])
                        ).configure_axis(labelLimit=1000))
        ...
        (AnalogClass
         .analog_container
         .markdown('Entries with no data available have been assigned '
