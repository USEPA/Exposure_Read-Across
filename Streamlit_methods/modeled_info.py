...
# However, in occasional situations, it was not possible to
...
class ModeledData:
    ...
    def minucci_loader(_self):
        ...
        Args:
            script_location (str): the location of the script that called the 
            module
        
        Returns:
            Dataframe: The Dataframe of the Minucci dataset.
        """
        # This version of consolidated_minucci has been compressed with the 
        # "snappy" algorithm, which is fast to retrieve.
        # Consolidated_minucci has been split into two parts so that they will 
        # be small enough to upload to github
    ...
    def ChemSTEER_udm(self, air_conc):
        ...
        # integer, days exposed per year; 250 is all the weekdays in a year
        ...
        # volumetric inhalation rate; 0 <= b <=7.9. The default value 
        # suggested by CLOET is 1.25 m^3/hr
        b = 1.25
    ...
    def qsur_display(self):
        ...
        (self
         ...
         .markdown("Reported Information is always a gold standard for data. "
                   "However, as is often the case with exposure science, many "
                   "data are lacking for the thousands of chemicals "
                   "humans interact with. For these cases, EPA and others have "
                   "developed *gap-filling* models to "
                   "predict data when no reported data are available. "
                   "Use of predicted information should acknowledge that these "
        ...
        (self
         ...
         .markdown('EPA Quantitative Structure-Use Relationship (QSUR) model '
                   'predictions. These are a set of machine-learning based '
                   'models '
                   'that predict uses for chemicals based on their structures.'
                   f'See [Phillips et al. 2017]({qsur_doi})'
        ...
        if predicted_info.empty:
            self.modeled_container.error('No Reported Function information '
                                         'available')
        else:
            (predicted_info
    ...
    def minucci_model(self):
        ...
        (self
         .modeled_container
         .markdown('This model predicts 1) if a chemical is likely to be '
                   'measured in a particular industrial sector and 2) '
                   'if so, what the likely air '
                   'concentration of the chemical in that industrial sector is'
                   '. More details can be found in '
                   f'[Minucci et al. 2021]({minnuci_doi}).'))
        minucci = self.minucci_loader()
        # Need to be able to share minucci_portion
        # with seem3_and_minucci method
    ...
    def seem3_and_minucci(self):
        ...
        top_cats['Quantity Type'] = 'Predicted Occupational ' \
        ...
        (self
         .modeled_container
         .markdown('SEEM3 uses a consensus modeling approach, combined with'
                   ' Bayesian methods to first predict possible exposure '
                   'pathways for a chemical and then predict the exposure '
                   'of the chemical for a general population. Predictions '
                   'from many different models (both developed by EPA and '
                   'by outside entities) are used to form '
                   'a consensus exposure prediction. For more details see '
                   f'[Ring et al. 2018]({ring_doi}).'))
        
        dat_sources = expo.search_exposures(by="seem",
        ...
        if not ModeledData.minucci_portion.empty or not dat_sources.empty:
            if ModeledData.minucci_portion.empty:
                ...
                combined_dat = pd.concat([dat_sources, Jeff_df], 
                                         ignore_index=True)
        ...
        (self
         .modeled_container
         .caption("Comparison of exposures predicted with exposure "
                  "doses calculated using the workplace air concentration "
                  "model from Minucci et al. 2021 and consensus "
                  "general population exposure calculated from "
                  "Ring et al. 2018. Workplace exposure doses are "
                  "calculated by using the predicted concentration "
                  "as input to ChemSTEER's User-Defined Inhalation model "
