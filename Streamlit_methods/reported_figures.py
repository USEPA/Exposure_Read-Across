...
class ReportedInfo:
    ...
    def functional_use(self):
        ...
        (self
         .reported_container
         .markdown('Reported consumer use information is '
                   'obtained from the EPA Chemicals and Products Database (CPDat) v4.0.'
                   'using CTX. To interactively explore this database, visit the '
                   f'[Chemical Exposure Knowledgebase (ChemExpo)]({chemexpo}).'))
        self.reported_container.info('Hover over any datapoint or bar on a figure '
        ...
        if function_info.empty:
            ...
            #Experimented with making this plot vertical, but decided that it looked 
            # worse, due to the bars being much shorter and running into the legend. I 
            # don't know if there is a way to move different components of the plot 
            # around
            ...
            (self
             .reported_container
             .markdown("Functional uses facilitates the understanding of the different"
                       " ways the substance may be used both in commerce as well as in "
                       "consumer products and industrial processes. For more "
                       "information about Function Categories and how they are used in "
                       f"CPDat, visit [the Function Categories page]({chemexpo}) on "
                       "ChemExpo. Data used to generated the figure below can be "
                       "obtained as a CSV file by clicking the `Download Data` button "
            ...
            (self
             ...
             .caption(f"The number of documents containing {self.structure_dtxsid} in "
                      ...
                      "count of documents having different reported functions are "
                      "shown by the differnt colored sections of the bar. Documents "
    ...
    def list_presence(self):
        ...
        (self
         .reported_container
         .markdown("List Presence Keywords inform the broad use of chemicals across "
                   "different sectors, products, geographic location, and even use in "
                   "regulatory or government documetns. For more information about "
                   "List Presence Keywords and how they are used in CPDat, visit the "
        ...
        if list_presence.empty:
            ...
            (self
             .reported_container
             .caption("The List Presence Keywords associated with documents that "
                      f"contain the chemical {self.structure_dtxsid} on which the "
    ...
    def product_inclusion(self):
        ...
        (self
         .reported_container
         .markdown("Product use categories provide information on how a consumer "
                   "product may be used. These categories have specifically been "
                   "developed for use in exposure modeling and assessment. For more "
                   "information about Product Use Categories and how they are "
                   f"used in CPDat, visit the [Product Use Categories page]({puc_url}) "
                   "on ChemExpo. Using the drop-down menu below, you can select the "
                   "PUC-level to use for aggregation. Checking the `Show all PUCs` box "
                   "below will limit the number of categories for display (if there is "
                   "a large number). Data used to generated this figure can be "
        ...
        if pucs_available.empty:
            ...
            try:
                ...
                puc_show_opts = (self
                                 .reported_container
                                 .selectbox(("Which level of PUC results would you "
                                             "like to see?"),
                                            ("General Category",
                ...
                (self
                 .reported_container
                 .markdown("PUC categories represented by only a small number of "
                           "products have been hidden. Check the `Show all PUCs` box"
                           "above to show them."))
                ...
                (self
                 ...
                 .caption("Number of unique CPDat products that contain "
                          f"{self.structure_dtxsid} and have been assigned a Product "
                          "Use Category (PUC)."))
                (self
                 .reported_container
            ...
            # that error was encountered without program halting
                ...
                (self
                 .reported_container
                 .error("Error encountered when retrieving product use information"))
    ...
    def osha_info(self):
        ...
        (self
         .reported_container
         .markdown("OSHA's Chemical Exposure Health Data (CEHD) provides industrial-"
                   "hygiene samples of chemicals measured in a workplace by OSHA "
                   "compliance officers from 1984 to 2022. While CEHD provides dermal "
                   "wipes and air measurements, only the air concentrations are "
                   "provided by the Exposure Data Viewer. These air concentration "
                   "measurements are taken by sampling either a worker's personal "
        ...
        if osha_of_interest.empty:
            ...
            self.reported_container.markdown('### OSHA Workplace Air Monitoring Data')
            (self
             .reported_container
             .download_button(label="Download Data",
            ...
            #Dropping entries with a value of zero, as these do not contain useful 
            # information
            ...
            naics_sector_or_sub = (self
                                   .reported_container
                                   .selectbox(("Which level of NAICS results would you "
                                              "like to see?"),
                                              ("NAICS Sector",
                                               "NAICS Subsector")))
            if not osha_of_interest.empty:
                (self
                 .reported_container
                 .markdown("#### Occupational inhalation exposure data on the entered "
                           "substance from filtered version of the dataset CEHD, "
                           "converted to units of mg/m^3"))
                if naics_sector_or_sub == "NAICS Subsector":
                    (self
                     .reported_container
                     .markdown("Data is separated by NAICS subsector. "
                               "Any entries with units that cannot be converted to "
                               "$mg/m^3$ or which have a non-inhalation sample type "
                               "are not displayed."))
                ...
                if naics_sector_or_sub == "NAICS Sector":
                    ...
                    (self
                     .reported_container
                     .markdown("Data is separated by NAICS sector. Any entries with "
                               "units that cannot be converted to mg/m^3 or which have "
                               "a non-inhalation sample type are not displayed."))
                    expo_range = [(osha_of_interest_nona['exposure_level'].min())/10,
                                  (osha_of_interest_nona['exposure_level'].max())*10]
                    box_plot = (alt
                                .Chart(osha_of_interest_nona)
                                .mark_boxplot()
                                .encode(x= (alt.X('exposure_level:Q',
                                                  scale=alt.Scale(type="log",
                                                                  domain=expo_range))
