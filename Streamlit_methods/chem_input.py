...
class FirstSection:
    ...
    def id_dialog(self, id_choice):
        ...
        if id_choice == "Structure":
            ...
            st.markdown("Draw the structure in the provided space below and then click "
                        "the `Apply` button to retrieve information on the chemical.")
        ...
        if id_choice == "DTXSID or CAS-RN":
            st.session_state.id_input = False
            st.markdown("Input a valid DTXSID or CAS-RN and then press the `enter` "
                        "keyboard key.")
    ...
    def substance_input(self):
        ...
        st.markdown('Choose chemical search method from the '
                    'drop-down menu below. To retrieve information on an different '
                    'chemical (or if the input box is not popping up after the input '
                    'type is selected), click the `Reset` button, then choose the '
            ...
            if smiles_code or id_entered:
                ...
                if len(hits) > 1 and type(hits)==list:
                    ...
                    st.warning("Multiple chemical matches are returned by CTX. "
                               "Information displayed below is from the best possible "
                               "match (as determined by CTX).")
                    #Select the dictionary with the highest rank
                    hits = min(hits, key=lambda d: d['rank'])
                    #code below is expecting hits to be in the form of a list of 
                    # dictionaries
                    hits = [hits]
                #All subsequent information is extracted by DTXSID, so the program 
                # cannot proceed if a DTXSID cannot be found
                elif len(hits) < 1:
                    st.warning("No information can be found on a chemical with this "
                               "structure or ID. Check that the structure or ID you "
                               "entered is correct.")  
                    raise Exception

                try:
                    #Lets the user know if the structure being used is a generic 
                    #structure for a class of compounds 
                    if hits[0]['isMarkush']:
                        st.warning("This chemical has a Markush SMILES string. This "
                                   "implies that there may be one or more isomeric "
                                   "matches to this structure. If you used the "
                                   "Structure search method to identify this compound, "
                                   "please ensure that the returned chemical "
                                   "information is for the correct chemical; "
                                   "otherwise, use the CompTox Chemicals Dashboard to "
                                   "search for your chemical and find the correct "
